...
FROM account_stats
GROUP BY CASE WHEN handle = 'your-handle' THEN 'your-handle' ELSE 'Competitors (avg)' END
```
...
WHERE c.handle = 'your-handle' AND c.visual_format IS NOT NULL
...
FROM content
WHERE handle = 'your-handle' AND spoken_hook IS NOT NULL
ORDER BY views DESC
LIMIT 10
```
...
SELECT
  ...
  SUM(CASE WHEN calc_outlier_category IN ('viral', 'hit') THEN 1 ELSE 0 END) as outliers
FROM content_with_scores
WHERE handle IN ('your-handle', 'COMPETITOR_HANDLE')
GROUP BY handle
```
...
FROM content
WHERE handle IN ('your-handle', 'COMPETITOR')
...
JOIN content c ON LOWER(TRIM(c.topic)) = LOWER(TRIM(n.topic))
  AND c.handle != 'your-handle'
WHERE n.handle = 'your-handle'
